...
def calibrate_single(imgNums, CheckerboardSize, Nx_cor, Ny_cor, saveFile=False, saveImages=False):
    ...
    for i in range(imgNums):
        frame = cv2.imread('CamCalib/img/' + str(i)+'.jpg', cv2.IMREAD_COLOR)
    ...
    if saveFile:
        np.savez('CamCalib/' + '/calibrate.npz', mtx=mtx, dist=dist, K=K, D=D)
    #cv2.destroyAllWindows()
    return mtx, dist, K, D

if __name__ == "__main__":
    mtx, dist, K,D = calibrate_single(37,25,11,8,True)
